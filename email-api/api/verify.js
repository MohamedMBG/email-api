...
export default async function handler(req, res) {
  ...
  // Verify JWT (must match JWT_SECRET used in /api/register)
  let email;
  try {
  ...
  // Init Admin (modular)
  const init = initAdminOrExplain();
  if (!init.ok) {
    return res.status(500).json({ ok: false, error: "admin init failed: " + init.error });
  }
